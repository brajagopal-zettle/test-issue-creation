...
createRelease() {
  ...
  gh api --method POST -H "Accept: application/vnd.github+json" /repos/"$REPO_OWNER"/"$PROJECT_REPONAME"/issues/"$ISSUE_NUMBER"/comments -f body="$RELEASE_CREATED_SUCCESS_MESSAGE $tag_name <p> Please check https://www.github.com/"$REPO_OWNER"/$PROJECT_REPONAME/releases"
}
